...
export class CreditCardPurchasesService {
      ...
      async (tx) => {
        ...
        if (pendingInstallments.length === 0) {
          throw new BadRequestException(
            'Não há parcelas pendentes para adiantar',
          );
        }
        ...
        if (dto.targetYear !== undefined && dto.targetMonth !== undefined) {
          ...
          if (!inPending) {
            const targetInv = await tx.creditCardInvoice.findUnique({
              where: {
  ...
  async remove(userId: string, id: string) {
    return this.prisma.withUser(userId, async (tx) => {
      ...
      const hasPaidInvoice = purchase.installments.some(
        (i) => i.invoice.isPaid,
      );
      if (hasPaidInvoice) {
        throw new BadRequestException(
      ...
      await tx.creditCardPurchase.delete({ where: { id, userId } });
